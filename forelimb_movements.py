...
class Joystick_analyzer:
    ...
    def pre_proccesing(self, cut_to = False, cut_first = False):
        ...
        for i,j in enumerate(self.gen_df):
           ...
           if cut_first:
               ...
               j.reset_index(inplace = True, drop = True)
           #j.to_excel("C:\\Users\\gniew\\Desktop\\PRACA\\Przemek\\test\\test.xlsx")
    ...
    def lick_histogram(self, pre_stim = 2, post_stim = 2, group = "all", marker= "r", smooth = True, window_length = 5, polyorder = 3, stat = "prob", sem = True, x_lim = False, y_lim = False, bin_ms = 50): 
       ...
       for l,df in enumerate(self.list_of_df):
            ...
            if self.opto:
                index_events = df.index[df['Event_Marker'] == 4].tolist()
            else:
       ...
       if group == "all":
           ...
           for index, row in df_licks_group.iterrows():
               ...
               if smooth:
                   ...
                   plt.annotate("Max", xy = (x[np.where(yhat == max(yhat))[0]][0], max(yhat)), xytext=(-1.0, max(yhat)),arrowprops = dict(facecolor='blue', shrink=0.1))
                   
                   plt.annotate("Reward start", xy = (0, 0), xytext=(0, (max(yhat) + min(yhat))/2),arrowprops = dict(facecolor='green', shrink=0.1))
                   plt.title("Smoothed")
                   plt.ylabel(y_label)
                   plt.xlabel("Time [s]")
                   plt.legend()
    ...
    def veloctiy(self, pre_stim = 2, post_stim = 2, group = "all", marker= "r", smooth = True, window_length = 5, polyorder = 3, sem = True, x_lim = False, y_lim = False, bin_ms = 50):
        ...
        for l,df in enumerate(self.list_of_df):
             ...
             for time_range in time_over_tresh_range:
                idx_ = [abs(df["Time_in_sec"]- time).idxmin() for time in time_range]
                if self.opto:
                    velocity = [df.iloc[index, 23] for index in idx_]
                else:
                    velocity = [df.iloc[index, 19] for index in idx_]
                    #time_check = [df.iloc[index, 0] for index in idx_]
                #print(time_check[-1]- time_check[0])
    ...
    def main_mov_process(self, data_holder):
        df_main_mov_all = pd.DataFrame(columns= ["Mice_id", "Move type", "Average move duration [s]", "Number of moves [n]", "Ratio move_type / move_all [%]"])
        for mice_id in data_holder["Mouse_ID"].unique():
            for move_type in data_holder["move type"].unique():
                ...
                [container.append(item) for item in [mice_id, move_type,  avg_duration, n_num, n_ratio]]
        ...
        for move_type in data_holder["move type"].unique():
            ...
            n_num = round(df_main_mov_all.loc[df_main_mov_all["Move type"] == move_type, "Number of moves [n]"].mean())
            n_ratio =round(df_main_mov_all.loc[df_main_mov_all["Move type"] == move_type, "Ratio move_type / move_all [%]"].mean(),2)
            [container.append(item) for item in ["Mean", move_type,  avg_duration, n_num, n_ratio]]
            df_main_mov_all.loc[len(df_main_mov_all)] = container
...
object_joy.pre_proccesing(cut_to=1200, cut_first=True)
object_joy.find_bugs(alfa = 0.1, automatic = False)
#object_joy.veloctiy(group = "mean", y_lim = [0, 5])
object_joy.amplitude(hue = "Event_Marker", event_markers = [1], group=None, x_lim= [5,20], y_lim=[0,27], x_axis = "mm", fill_nan= False) 
...
#object_joy.trajectory(move_range = "start_move_to_max", calibrate= False, polar = False, x_lim=[300,750], y_lim=[-750,-300], y_lim_polar=[0,26], delta= True, delta_n= 5)
