...
class Joystick_analyzer:
    def __init__(self, opto = False, close = False):
        self.opto = opto
        self.close = close
        if self.opto:
            print("--------> Opto mode activated <--------")
            self.column_name = ["Time_in_sec", "Event_Marker", "TrialCt", "JoyPos_X", "JoyPos_Y", "Amplitude_Pos", "Base_JoyPos_X",
            "Base_JoyPos_Y", "SolOpenDuration", "DelayToRew", "ITI", "Threshold", "Fail_attempts", "Sum_of_fail_attempts", 
            "Lick_state", "Sum_licks",  "stim", "NormalTrialCt", "StimTrialCt", "PostStimTrialCt", "opto_state"]
            ...
            self.list_of_files = [[i +"_prestim", i +"_stim", i + "_poststim"] for i in  self.list_of_files]
            ...
            self.list_of_files_prestim = [i for i in self.list_of_files if "_prestim" in i]
            self.list_of_files_poststim = [i for i in self.list_of_files if "_poststim" in i]
    ...
    def pre_proccesing(self, cut_to = False, cut_first = False):
        ...
        for i,j in enumerate(self.gen_df):
           if i == 0:
               self.time_ms = j.iloc[:,0]
           if not self.opto:
               if j.shape[1] == 17:
                   j = j.iloc[:, :-1]
           j.iloc[:, 0] = j.iloc[:, 0].apply(lambda x : round((x / 1000),4)) # Convert time to seconds
           j.columns = self.column_name
          
           max_row = j["Time_in_sec"].tolist()
           max_row = [round(i) for i in max_row]
           trial_max = j["TrialCt"].unique()
           bugged_index = [j.loc[j["TrialCt"] == l, "Event_Marker"].lt(0).idxmax() for l in trial_max]
           
           j.iloc[bugged_index, 5] = 0
           
           j["Amplitude_Pos_mm" ] = j["Amplitude_Pos"].apply(lambda x: x/ u_to_mm)
           delta_dist = j["Amplitude_Pos_mm"].tolist()
           ...
           delta_dist_negative = j["Amplitude_Pos_mm"].tolist()
           ...
           j["delta_neg_Amplitude_Pos_mm"] = delta_dist_negative
           
           time_list = j["Time_in_sec"].tolist()
           ...
           if cut_to:
               df_sort = j.iloc[(j["Time_in_sec"]- cut_to).abs().argsort()[:1]]
           ...
           if self.opto:
               stim_start_index = j.loc[j["opto_state"]==1].first_valid_index()
               stim_stop_index = j.loc[j["opto_state"]==1].last_valid_index()
               
               j_prestim = j.iloc[:stim_start_index, :]
               j_prestim = j_prestim.loc[j_prestim["TrialCt"] != int(j.loc[stim_start_index -1, "TrialCt"]), :]
               j_prestim.reset_index(inplace = True, drop = True)
               self.list_of_df.append(j_prestim)
               j_stim = j.iloc[stim_start_index:stim_stop_index+1, :]
               j_stim = j_stim.loc[(j_stim["TrialCt"] != int(j.loc[stim_start_index , "TrialCt"])) & (j_stim["TrialCt"] != int(j.loc[stim_stop_index, "TrialCt"])), :]              
               j_stim.reset_index(inplace = True, drop = True)
               self.list_of_df.append(j_stim)
               
               j_poststim = j.iloc[stim_stop_index+1:,:]
               j_poststim = j_poststim.loc[j_poststim["TrialCt"] != int(j.loc[stim_stop_index +1, "TrialCt"]), :]
               j_poststim.reset_index(inplace = True, drop = True)
               self.list_of_df.append(j_poststim)
               
               #j_prestim.to_excel("C:\\Users\\malgo\\OneDrive\\Pulpit\\GD_MB\Przemek\\test_2.xlsx")
               #j_poststim.to_excel("C:\\Users\\gniew\\Desktop\\Praca\GD_MB\\Przemek\\test.xlsx")
               
               [self.trials_conteiner.append(sorted(set(df_j.loc[:, "TrialCt"].tolist()))) for df_j in [j_prestim, j_stim, j_poststim]]
               [self.new_max_v2.append(len(set(df_j.loc[:, "TrialCt"].tolist()))) for df_j in [j_prestim, j_stim, j_poststim]]
    ...
    def amplitude(self, event_markers = [0,1,2,3,4], hue = None, kde = False, group = "Mouse_ID", fill_nan = False, y_stat = "count", x_axis = "units", x_lim = False, y_lim = False, drop_nan = True):
                ...
                for k in event_markers:
                    if x_axis == "units":
                        if self.opto:
                            ...
                            amplitude_ = [i.loc[(i["TrialCt"] == j) & (i["Event_Marker"] == k), "Amplitude_Pos"].max() for j in range(0, trial_max + 1)]
                            mouse_id = [self.list_of_files[l] for m in range(0, trial_max + 1)]
                            event_marker = [k for j in range(0, trial_max + 1)]
                            dict_to_add = {"TrialCt": range(0, trial_max + 1), "Mouse_ID": mouse_id, "Amplitude_Pos": amplitude_, "Event_Marker": event_marker}
                            df_amplitude = df_amplitude.append(pd.DataFrame(dict_to_add))
                            df_amplitude.reset_index(inplace = True, drop = True)
                    elif x_axis == "mm":
                        if self.opto:
                            ...
                            amplitude_ = [i.loc[(i["TrialCt"] == j) & (i["Event_Marker"] == k), "Amplitude_Pos_mm"].max() for j in range(0, trial_max + 1)]
                            mouse_id = [self.list_of_files[l] for m in range(0, trial_max + 1)]
                            event_marker = [k for j in range(0, trial_max + 1)]
                            dict_to_add = {"TrialCt": range(0, trial_max + 1), "Mouse_ID": mouse_id, "Amplitude_Pos_mm": amplitude_, "Event_Marker": event_marker}
                            df_amplitude = df_amplitude.append(pd.DataFrame(dict_to_add))
                            df_amplitude.reset_index(inplace = True, drop = True)
        ...
        if drop_nan:
            df_amplitude.dropna(inplace= True)
        ...
        if group == None:
            if x_axis == "units":
                if self.opto:
                    final_df_prestim = self.__calculate_mean_count(df_amplitude= df_amplitude, group= self.list_of_files_prestim, units= "Amplitude_Pos", event_markers= event_markers, bins= bins)
                    final_df_stim = self.__calculate_mean_count(df_amplitude= df_amplitude, group= self.list_of_files_stim, units= "Amplitude_Pos", event_markers= event_markers, bins= bins)
                    final_df_poststim = self.__calculate_mean_count(df_amplitude= df_amplitude, group= self.list_of_files_poststim, units= "Amplitude_Pos", event_markers= event_markers, bins= bins)
                    
                    fig, axs = plt.subplots(3,figsize = (16,9), constrained_layout=True)
                    #fig.tight_layout()
                    sns.barplot(data = final_df_prestim, x="Amplitude_Pos", y="Value", color='skyblue', edgecolor='black', ax= axs[0], errorbar="se").set(title = "Pre-stimulation")
                    sns.barplot(data = final_df_stim, x="Amplitude_Pos", y="Value", color='skyblue', edgecolor='black', ax= axs[1], errorbar="se").set(title = "Stimulation")
                    sns.barplot(data = final_df_poststim, x="Amplitude_Pos", y="Value", color='skyblue', edgecolor='black', ax= axs[2], errorbar="se").set(title = "Post-stimulation")
                    ...
                    if x_lim:
                        axs[0].set_xlim(x_lim[0]-1, x_lim[1])
                        axs[1].set_xlim(x_lim[0]-1, x_lim[1])
                        axs[2].set_xlim(x_lim[0]-1, x_lim[1])
                    if y_lim:
                        axs[0].set_ylim(y_lim[0], y_lim[1])
                        axs[1].set_ylim(y_lim[0], y_lim[1])
                        axs[2].set_ylim(y_lim[0], y_lim[1])   
                    plt.legend(labels=[f"n = {len(self.list_of_files_prestim)}"])       
                else:
                ...
                if self.opto:
                     final_df_prestim = self.__calculate_mean_count(df_amplitude= df_amplitude, group= self.list_of_files_prestim, units= "Amplitude_Pos_mm", event_markers= event_markers, bins= bins)
                     final_df_stim = self.__calculate_mean_count(df_amplitude= df_amplitude, group= self.list_of_files_stim, units= "Amplitude_Pos_mm", event_markers= event_markers, bins= bins)
                     final_df_poststim = self.__calculate_mean_count(df_amplitude= df_amplitude, group= self.list_of_files_poststim, units= "Amplitude_Pos_mm", event_markers= event_markers, bins= bins)
                     
                     fig, axs = plt.subplots(3,figsize = (16,9), constrained_layout=True)
                     #fig.tight_layout()
                     sns.barplot(data = final_df_prestim, x="Amplitude_Pos_mm", y="Value", color='skyblue', edgecolor='black', ax= axs[0], errorbar="se").set(title = "Pre-stimulation")
                     sns.barplot(data = final_df_stim, x="Amplitude_Pos_mm", y="Value", color='skyblue', edgecolor='black', ax= axs[1], errorbar="se").set(title = "Stimulation")
                     sns.barplot(data = final_df_poststim, x="Amplitude_Pos_mm", y="Value", color='skyblue', edgecolor='black', ax= axs[2], errorbar="se").set(title = "Post-stimulation")
                     axs[0].set_ylabel("Mean count") 
                     axs[1].set_ylabel("Mean count")
                     axs[2].set_ylabel("Mean count")
        ...
        if msg == "yes":
            ...
            if self.close:
                ...
                plt.pause(1)  # Keep open for 3 seconds
                plt.close()
            else:
                plt.show()
    ...
    def __calculate_mean_count(self, df_amplitude, group, units, event_markers, bins):
        bin_centers = [(bins[i] + bins[i+1]) / 2 for i in range(len(bins) - 1)]
                        
        data_names = [list(Counter(list(df_amplitude.loc[(df_amplitude[units] >= bins[ii]) & (df_amplitude[units] < bins[ii+1]) & (df_amplitude["Event_Marker"].isin(event_markers)) & (df_amplitude["Mouse_ID"].isin(group)),  "Mouse_ID"])).values()) for ii in range(0, len(bins)-1)]
        bin_names = [list(df_amplitude.loc[(df_amplitude[units] >= bins[ii]) & (df_amplitude[units] < bins[ii+1]) & (df_amplitude["Event_Marker"].isin(event_markers)) & (df_amplitude["Mouse_ID"].isin(group)),  "Mouse_ID"]) for ii in range(0, len(bins)-1)]
        bin_names__ = []
        for names in bin_names:
            ans = [w for w in self.list_of_files_prestim if w not in names]
            ans_2 = [0 for w in ans]
            bin_names__.append(ans_2)
        final_data = [w + y for w, y in zip(bin_names__, data_names)]
        
        values = []
        groups = []
        for i, sublist in enumerate(final_data):

            for val in sublist:
                values.append(val)
                groups.append(bin_centers[i])
        final_df = pd.DataFrame({units: groups, "Value": values})
        return final_df
    
    def lick_histogram(self, pre_stim = 2, post_stim = 2, group = "all", marker= "r", smooth = True, window_length = 5, polyorder = 3, stat = "prob", sem = True, x_lim = False, y_lim = False, bin_ms = 50): 
       global df_result_lick, switcher, xd, df_lick_counts
       
       
       assert len(self.list_of_df) ==  len(self.list_of_files)
       bin_n = int((pre_stim + post_stim) / (bin_ms / 1000))
       columns = np.linspace(start = -pre_stim, stop = post_stim, num = bin_n + 1)
       x = [round(i,3) for i in columns]
       columns_=   [str(round(i,3)) for i in columns]
       columns_v2 = np.linspace(start = -pre_stim, stop = post_stim, num = bin_n + 1)
       columns_v2=   [str(round(i,3)) for i in columns_v2]
       columns_v2.append("Animal_ID")
       x = np.array(x)
       #sem_velocity_list = []
       if self.opto:
           df_licks_group = pd.DataFrame(columns =  columns_v2, index = [i for i in range(0, len(self.list_of_df)+3)])
       else:
           df_licks_group = pd.DataFrame(columns =  columns_v2, index = [i for i in range(0, len(self.list_of_df)+1)])
       sem_lick_group = []
       for l,df in enumerate(self.list_of_df):
            lick_index = df.columns.tolist()
            lick_index = int(lick_index.index('Lick_state'))     
            index_events = df.index[df['Event_Marker'] == 2].tolist()
            ...
            if switcher:
                ...
                trial_max = df["TrialCt"].max()
            time_over_tresh = [i for i in index_events if i != None]
            time_over_tresh = [df.iloc[i, 0] for i in time_over_tresh]
            time_over_tresh_range = [x + i for i in time_over_tresh]
             
            df_lick = pd.DataFrame(columns= columns_)
             
            for time_range in time_over_tresh_range:
                idx_ = [abs(df["Time_in_sec"]- time).idxmin() for time in time_range]
                lick = [df.iloc[index,lick_index] for index in idx_]
                    #time_check = [df.iloc[index, 0] for index in idx_]
                #print(time_check[-1]- time_check[0])
                assert len(lick) == len(x)
                df_lick.loc[len(df_lick)] =  lick
            
            lick_rate = df_lick
            prob_lick = df_lick.apply(lambda x: x.value_counts())
            ...
            if stat == "prob":
                ...
                for i in range(0, lick_rate.shape[1]):
                    lick_rate.iloc[:,i] = lick_rate.iloc[:,i].apply(lambda x: (x * 1000)/bin_ms) 
                if sem:
       ...
       if self.opto:
           df_licks_group.loc[len(df_licks_group)-3, "Animal_ID"] = "Mean_prestim"
           df_licks_group.loc[len(df_licks_group)-2, "Animal_ID"] = "Mean_stim"
           df_licks_group.loc[len(df_licks_group)-1, "Animal_ID"] = "Mean_poststim"
       else:
           df_licks_group.iloc[len(self.list_of_df), 0: len(columns_v2)-1] = df_licks_group.mean()
           df_licks_group.loc[len(self.list_of_df), "Animal_ID"] = "Mean"
       df_licks_group.set_index("Animal_ID", inplace = True)
       if self.opto:
           columns_prestim = [str(column) for column in df_licks_group.index.values.tolist() if "_prestim" in str(column)]
           df_licks_group.loc["Mean_prestim", :] = df_licks_group.loc[columns_prestim, :].mean()
           ...
           columns_poststim = [str(column) for column in df_licks_group.index.values.tolist() if "_poststim" in str(column)]
           df_licks_group.loc["Mean_poststim", :] = df_licks_group.loc[columns_poststim, :].mean()
           sem_lick_group.append(df_licks_group.loc[columns_prestim, :].sem().tolist())
           sem_lick_group.append(df_licks_group.loc[columns_stim, :].sem().tolist())
           sem_lick_group.append(df_licks_group.loc[columns_poststim, :].sem().tolist())
       ...
       if group == "all":
           ...
           for index, row in df_licks_group.iterrows():
               ...
               plt.ylabel(y_label)
               plt.xlabel("Time [s]")
               plt.annotate("Max", xy = (float(row[row == row.max()].index[0]), row.max()), xytext=(-1.0, row.max()),arrowprops = dict(facecolor='blue'))
               plt.annotate("Reward start", xy = (0, row.min()), xytext=(0, (row.max() + row.min())/2),arrowprops = dict(facecolor='green'))
               plt.legend()
               ...
               if smooth:
                   ...
                   plt.annotate("Max", xy = (x[np.where(yhat == max(yhat))[0]][0], max(yhat)), xytext=(-1.0, max(yhat)),arrowprops = dict(facecolor='blue'))
                   
                   plt.annotate("Reward start", xy = (0, 0), xytext=(0, (max(yhat) + min(yhat))/2),arrowprops = dict(facecolor='green'))
                   plt.title("Smoothed")
                   plt.ylabel(y_label)
                   plt.xlabel("Time [s]")
                   plt.legend()
           ...
           if not self.opto:
                ...
                if smooth:
                    ...
                    if stat == "rate":
                        ...
                        if y_lim:
                            plt.ylim(y_lim[0], y_lim[1])
                    plt.annotate("Max", xy = (x[np.where(yhat == max(yhat))[0]], max(yhat)), xytext=(-1.0, max(yhat)),arrowprops = dict(facecolor='blue'))
                    plt.annotate("Reward start", xy = (0, min(yhat)), xytext=(0, max(yhat)/2),arrowprops = dict(facecolor='green'))
               ...
               y_Mean_prestim = np.array(df_licks_group.iloc[-3])
               y_Mean_stim = np.array(df_licks_group.iloc[-2])
               y_Mean_poststim = np.array(df_licks_group.iloc[-1])
               if stat == "rate" and sem:
                    error_sem_prestim = np.array(sem_lick_group[-3])
                    error_sem_stim = np.array(sem_lick_group[-2])
                    error_sem_poststim = np.array(sem_lick_group[-1])
                    
                    upper_band_prestim = np.asfarray(y_Mean_prestim + error_sem_prestim)
                    lower_band_prestim = np.asfarray(y_Mean_prestim - error_sem_prestim)
                    upper_band_stim = np.asfarray(y_Mean_stim + error_sem_stim)
                    lower_band_stim = np.asfarray(y_Mean_stim - error_sem_stim)
                    upper_band_poststim = np.asfarray(y_Mean_poststim + error_sem_poststim)
                    lower_band_poststim = np.asfarray(y_Mean_poststim - error_sem_poststim)
                    
                    axs[0].fill_between(x,  upper_band_prestim, lower_band_prestim, alpha = 0.4, color = "r")
                    axs[1].fill_between(x, upper_band_stim, lower_band_stim, alpha = 0.4, color = "r")
                    axs[2].fill_between(x, upper_band_poststim, lower_band_poststim, alpha = 0.4, color = "r")
               if stat == "rate":
                    if x_lim:
                         axs[0].set_xlim(x_lim[0], x_lim[1])
                         axs[1].set_xlim(x_lim[0], x_lim[1])
                         axs[2].set_xlim(x_lim[0], x_lim[1])
               ...
               axs[0].plot(x,df_licks_group.iloc[-3], marker, label = "Mean_lick") 
               axs[1].plot(x,df_licks_group.iloc[-2], marker, label = "Mean_lick")
               axs[2].plot(x,df_licks_group.iloc[-1], marker, label = "Mean_lick")
               fig.suptitle("Original")
               axs[0].set_title('Mean_prestim')
               axs[1].set_title('Mean_stim')
               axs[2].set_title('Mean_poststim')
               ...
               axs[0].annotate("Max", xy = (float(df_licks_group.iloc[-3][df_licks_group.iloc[-3] == df_licks_group.iloc[-3].max()].index[0]), df_licks_group.iloc[-3].max()), xytext=(-1.0, df_licks_group.iloc[-3].max()),arrowprops = dict(facecolor='blue'))
               axs[0].annotate("Reward start", xy = (0, df_licks_group.iloc[-3].min()), xytext=(0, (df_licks_group.iloc[-3].max() + df_licks_group.iloc[-3].min())/2),arrowprops = dict(facecolor='green', ))   
               axs[1].annotate("Max", xy = (float(df_licks_group.iloc[-2][df_licks_group.iloc[-2] == df_licks_group.iloc[-2].max()].index[0]), df_licks_group.iloc[-2].max()), xytext=(-1.0, df_licks_group.iloc[-2].max()),arrowprops = dict(facecolor='blue', ))
               axs[1].annotate("Reward start", xy = (0, df_licks_group.iloc[-2].min()), xytext=(0, (df_licks_group.iloc[-2].max() + df_licks_group.iloc[-2].min())/2),arrowprops = dict(facecolor='green',))
               axs[2].annotate("Max", xy = (float(df_licks_group.iloc[-1][df_licks_group.iloc[-1] == df_licks_group.iloc[-1].max()].index[0]), df_licks_group.iloc[-1].max()), xytext=(-1.0, df_licks_group.iloc[-1].max()),arrowprops = dict(facecolor='blue', ))
               axs[2].annotate("Reward start", xy = (0, df_licks_group.iloc[-1].min()), xytext=(0, (df_licks_group.iloc[-1].max() + df_licks_group.iloc[-1].min())/2),arrowprops = dict(facecolor='green', ))
               ...
               fig.tight_layout()
           if msg == "yes":
                save_file_v3 = save_file_v1 + "//" + "Mean" + "_" + "orginal_lick_opto" + ".svg"
           ...
           if self.opto and smooth:
               fig, axs = plt.subplots(3)
               yhat_prestim = savgol_filter(np.array(df_licks_group.iloc[-3]), window_length, polyorder)
               yhat_stim = savgol_filter(np.array(df_licks_group.iloc[-2]), window_length, polyorder)
               yhat_poststim = savgol_filter(np.array(df_licks_group.iloc[-1]), window_length, polyorder)
               if sem and stat == "rate":
                   yhat_error_sem_prestim = savgol_filter(np.array(sem_lick_group[-3]), window_length, polyorder)
                   yhat_error_sem_stim = savgol_filter(np.array(sem_lick_group[-2]), window_length, polyorder)
                   yhat_error_sem_poststim = savgol_filter(np.array(sem_lick_group[-1]), window_length, polyorder)

                   upper_band_prestim_sem = np.asfarray(yhat_prestim + yhat_error_sem_prestim)
                   lower_band_prestim_sem = np.asfarray(yhat_prestim - yhat_error_sem_prestim)
                   ...
                   upper_band_poststim_sem = np.asfarray(yhat_poststim + yhat_error_sem_poststim)
                   lower_band_poststim_sem = np.asfarray(yhat_poststim - yhat_error_sem_poststim)
                   
                   axs[0].fill_between(x, upper_band_prestim_sem, lower_band_prestim_sem, alpha = 0.4, color = "r")
                   axs[1].fill_between(x, upper_band_stim_sem, lower_band_stim_sem, alpha = 0.4, color = "r")
                   axs[2].fill_between(x, upper_band_poststim_sem, lower_band_poststim_sem, alpha = 0.4, color = "r")
               if stat == "rate":
                    if x_lim:
                         axs[0].set_xlim(x_lim[0], x_lim[1])
                         axs[1].set_xlim(x_lim[0], x_lim[1])
                    ...
                    if y_lim:
                         ...
                         axs[1].set_ylim(y_lim[0], y_lim[1])
                         axs[2].set_ylim(y_lim[0], y_lim[1])
               axs[0].plot(x,yhat_prestim, marker, label = "Mean_lick")
               axs[1].plot(x,yhat_stim, marker, label = "Mean_lick")
               axs[2].plot(x,yhat_poststim, marker, label = "Mean_lick")
               
               fig.suptitle("Smoothed")
               axs[0].set_title('Prestimulation')
               axs[1].set_title('Stimulation')
               axs[2].set_title('Poststimulation')
               axs[0].set_ylabel(y_label)
               axs[0].set_xlabel("Time [s]")
               axs[1].set_ylabel(y_label)
               axs[1].set_xlabel("Time [s]")
               ...
               axs[0].annotate("Max", xy = (x[np.where(yhat_prestim == max(yhat_prestim))[0]], max(yhat_prestim)), xytext=(-1.0, max(yhat_prestim)),arrowprops = dict(facecolor='blue'))
               axs[0].annotate("Reward start", xy = (0, yhat_prestim.min()), xytext=(0, (yhat_prestim.max() + yhat_prestim.min())/2),arrowprops = dict(facecolor='green'))
               
               axs[1].annotate("Max", xy = (x[np.where(yhat_stim == max(yhat_stim))[0]], max(yhat_stim)), xytext=(-1.0, max(yhat_stim)),arrowprops = dict(facecolor='blue'))
               axs[1].annotate("Reward start", xy = (0, yhat_stim.min()), xytext=(0, (yhat_stim.max() + yhat_stim.min())/2),arrowprops = dict(facecolor='green'))
               
               axs[2].annotate("Max", xy = (x[np.where(yhat_poststim == max(yhat_poststim))[0]], max(yhat_poststim)), xytext=(-1.0, max(yhat_poststim)),arrowprops = dict(facecolor='blue'))
               axs[2].annotate("Reward start", xy = (0, yhat_poststim.min()), xytext=(0, (yhat_poststim.max() + yhat_poststim.min())/2),arrowprops = dict(facecolor='green'))
    ...
    def trajectory(self, move_range = "0_to_max", calibrate = False, x_lim = False, y_lim = False, polar = False, starting_point = True, y_lim_polar = False, delta = False, delta_n = 5):
        ...
        delta_index = self.list_of_df[0].columns.tolist()
        delta_index = int(delta_index.index("delta_Amplitude_Pos_mm"))     
        for l,i in enumerate(self.list_of_df):
            ...
            if delta:
                ...
                for range_index in zip(movment_end, start_index):
                    for current_index in range(range_index[0], range_index[1]-1,-1):
                        current_delta = i.iloc[current_index, delta_index]
                        ...
                        if (i.iloc[current_index, 5] < 10 and i.iloc[current_index, 1] == 0) or  n > delta_n :
            ...
            #print(self.list_of_files[l])
            #ddd = [print(list_x) for list_x in movment_list_x_0_to_max]
            ...
            if move_range == "start_move_to_max":
                ...
                #[print(lll, index) for index, lll in enumerate(zip(movment_list_x_0_to_max, movment_list_y_0_to_max))]
                ...
                axs[0].set_title("Trajectory plot")
                uni_list_x = []
                uni_list_y = []
                ...
                try:
                    ...
                    print("Density plot can not be created for this data")    
                if calibrate:
            ...
            if calibrate and polar:
                ...
                ax.set_rticks([2,6,10,14,18,22,26])
            ...
            move_type = []
            for move_ in movment_list_y_0_to_max_backup:
                if move_[-1] - move_[0] < -10 :
                    move_type.append("Push")
                elif move_[-1] - move_[0] > 10:
                    move_type.append("Pull")
                else:
                    move_type.append("Left/right")
                    

            dict_to_add = {"TrialCt": trial_list_v2, "Mouse_ID": self.list_of_files[l], "x_coordinate_start_move_to_max": movment_list_x_0_to_max_backup, "y_coordinate_start_move_to_max": movment_list_y_0_to_max_backup, "move_duration [sec]": movement_duration, "time_trial_start [sec]": trial_time_start, "time_start [sec]": time_of_movement_start, "time_end [sec]": time_of_movement_stop, "move_duration_from_trial_start [sec]" : movement_duration_from_trial, "move type": move_type}
            df_trajectory = df_trajectory.append(pd.DataFrame(dict_to_add))
            df_trajectory["move latency [sec]"] =  df_trajectory["time_start [sec]"] - df_trajectory["time_trial_start [sec]"]
            df_trajectory_summary = self.main_mov_process(df_trajectory)
        

        if msg == "yes":
            df_trajectory.to_excel(save_file_v1 + "//" + self.list_of_files[0] + "_" + self.list_of_files[-1] + "_trajectory_raw" + ".xlsx")
            df_trajectory_summary.to_excel(save_file_v1 + "//" + self.list_of_files[0] + "_" + self.list_of_files[-1] + "_trajectory_summary" + ".xlsx")
            if calibrate and polar:
    ...
    def veloctiy(self, pre_stim = 2, post_stim = 2, group = "all", marker= "r", smooth = True, window_length = 5, polyorder = 3, sem = True, x_lim = False, y_lim = False, bin_ms = 50):                                                                                            
        ...
        sem_velocity_list = []
        if self.opto:
           df_velocity_group = pd.DataFrame(columns = columns_v2, index = [i for i in range(0, len(self.list_of_df)+3)])
        else:
           df_velocity_group = pd.DataFrame(columns = columns_v2, index = [i for i in range(0, len(self.list_of_df)+1)])
    
        for l,df in enumerate(self.list_of_df):
             velocity_index = df.columns.tolist()
             ...
             df_velocity = pd.DataFrame(columns= columns_)
             
             for time_range in time_over_tresh_range:
                idx_ = [abs(df["Time_in_sec"]- time).idxmin() for time in time_range]
                velocity = [df.iloc[index, velocity_index] for index in idx_]
                    #time_check = [df.iloc[index, 0] for index in idx_]
                #print(time_check[-1]- time_check[0])
                assert len(velocity) == len(x)
                df_velocity.loc[len(df_velocity)] =  velocity
        ...
        if self.opto:
           df_velocity_group.loc[len(df_velocity_group)-3, "Animal_ID"] = "Mean_prestim"
           df_velocity_group.loc[len(df_velocity_group)-2, "Animal_ID"] = "Mean_stim"
           df_velocity_group.loc[len(df_velocity_group)-1, "Animal_ID"] = "Mean_poststim"
        
        
        else:
            df_velocity_group.iloc[len(self.list_of_df), 0: len(columns_v2)-1] = df_velocity_group.mean()
            df_velocity_group.loc[len(self.list_of_df), "Animal_ID"] = "Mean"
        df_velocity_group.set_index("Animal_ID", inplace = True)
        if self.opto:
           
           columns_prestim = [str(column) for column in df_velocity_group.index.values.tolist() if "_prestim" in str(column)]
           df_velocity_group.loc["Mean_prestim", :] = df_velocity_group.loc[columns_prestim, :].mean()
           ...
           columns_poststim = [str(column) for column in df_velocity_group.index.values.tolist() if "_poststim" in str(column)]
           df_velocity_group.loc["Mean_poststim", :] = df_velocity_group.loc[columns_poststim, :].mean()
           
           sem_velocity_list.append(df_velocity_group.loc[columns_prestim, :].sem().tolist())
           sem_velocity_list.append(df_velocity_group.loc[columns_stim, :].sem().tolist())
           sem_velocity_list.append(df_velocity_group.loc[columns_poststim, :].sem().tolist())      
        main = tk.Tk()
        msg = tk.messagebox.askquestion ('Save window','Do you want to save graphs and data?',icon = 'warning')
        if msg == "yes":
            main.destroy()
        ...
        if group == "all":
            ...
            if not self.opto:
               ...
               fig, axs = plt.subplots(3)
               y_Mean_prestim = np.array(df_velocity_group.iloc[-3])
               y_Mean_stim = np.array(df_velocity_group.iloc[-2])
               y_Mean_poststim = np.array(df_velocity_group.iloc[-1])
               if sem:
                    error_sem_prestim = np.array(sem_velocity_list[-3])
                    error_sem_stim = np.array(sem_velocity_list[-2])
                    error_sem_poststim = np.array(sem_velocity_list[-1])
                    upper_band_prestim = np.asfarray(y_Mean_prestim + error_sem_prestim)
                    lower_band_prestim = np.asfarray(y_Mean_prestim - error_sem_prestim)
                    upper_band_stim = np.asfarray(y_Mean_stim + error_sem_stim)
                    lower_band_stim = np.asfarray(y_Mean_stim - error_sem_stim)
               ...
               if y_lim:
                    ...
                    axs[1].set_ylim(y_lim[0], y_lim[1])
                    axs[2].set_ylim(y_lim[0], y_lim[1])
               axs[0].plot(x,df_velocity_group.iloc[-3], marker, label = "Mean_velocity")
               axs[1].plot(x,df_velocity_group.iloc[-2], marker, label = "Mean_velocity")
               axs[2].plot(x,df_velocity_group.iloc[-1], marker, label = "Mean_velocity")
               fig.suptitle("Original")
               axs[0].set_title('Pre-stimulation')
               axs[1].set_title('Stimulation')
               axs[2].set_title('Post-stimulation')
               axs[0].set_ylabel(y_label)
               axs[0].set_xlabel("Time [s]")
               axs[1].set_ylabel(y_label)
               axs[1].set_xlabel("Time [s]")
               axs[2].set_ylabel(y_label)
               axs[2].set_xlabel("Time [s]")
               axs[0].annotate("Max", xy = (float(df_velocity_group.iloc[-3][df_velocity_group.iloc[-3] == df_velocity_group.iloc[-3].max()].index[0]), df_velocity_group.iloc[-3].max()), xytext=(-1.0, df_velocity_group.iloc[-3].max()),arrowprops = dict(facecolor='blue', ))
               axs[0].annotate("Reward start", xy = (0, df_velocity_group.iloc[-3].min()), xytext=(0, (df_velocity_group.iloc[-3].max() + df_velocity_group.iloc[-3].min())/2),arrowprops = dict(facecolor='green', ))
               
               axs[1].annotate("Max", xy = (float(df_velocity_group.iloc[-2][df_velocity_group.iloc[-2] == df_velocity_group.iloc[-2].max()].index[0]), df_velocity_group.iloc[-2].max()), xytext=(-1.0, df_velocity_group.iloc[-2].max()),arrowprops = dict(facecolor='blue', ))
               axs[1].annotate("Reward start", xy = (0, df_velocity_group.iloc[-2].min()), xytext=(0, (df_velocity_group.iloc[-2].max() + df_velocity_group.iloc[-2].min())/2),arrowprops = dict(facecolor='green', ))
               
               axs[2].annotate("Max", xy = (float(df_velocity_group.iloc[-1][df_velocity_group.iloc[-1] == df_velocity_group.iloc[-1].max()].index[0]), df_velocity_group.iloc[-1].max()), xytext=(-1.0, df_velocity_group.iloc[-1].max()),arrowprops = dict(facecolor='blue', ))
               axs[2].annotate("Reward start", xy = (0, df_velocity_group.iloc[-1].min()), xytext=(0, (df_velocity_group.iloc[-1].max() + df_velocity_group.iloc[-1].min())/2),arrowprops = dict(facecolor='green', ))
               ...
               if msg == "yes":
                    save_file_v5 = save_file_v1 + "//" + "Mean" + "_" + "orginal_velocity" + ".svg"
                    plt.savefig(save_file_v5)
                    if self.close:
                            plt.show(block=False)  # Show without blocking execution
                            plt.pause(1)  # Keep open for 3 seconds
            ...
            if self.opto and smooth:
               fig, axs = plt.subplots(3)
               yhat_prestim = savgol_filter(np.array(df_velocity_group.iloc[-3]), window_length, polyorder)
               yhat_stim = savgol_filter(np.array(df_velocity_group.iloc[-2]), window_length, polyorder)
               yhat_poststim = savgol_filter(np.array(df_velocity_group.iloc[-1]), window_length, polyorder)
               if sem :
                   yhat_error_sem_prestim = savgol_filter(np.array(sem_velocity_list[-3]), window_length, polyorder)
                   yhat_error_sem_stim = savgol_filter(np.array(sem_velocity_list[-2]), window_length, polyorder)
                   yhat_error_sem_poststim = savgol_filter(np.array(sem_velocity_list[-1]), window_length, polyorder)
                   ...
                   if y_lim:
                         ...
                         axs[1].set_ylim(y_lim[0], y_lim[1])
                         axs[2].set_ylim(y_lim[0], y_lim[1])
               axs[0].plot(x,yhat_prestim, marker, label = "Mean_velocity")
               axs[1].plot(x,yhat_stim, marker, label = "Mean_velocity")
               axs[2].plot(x,yhat_poststim, marker, label = "Mean_velocity")
               ...
               if msg == "yes":
                    save_file_v6 = save_file_v1 + "//" + "Mean" + "_" + "smoothed_velocity_" + ".svg"
                    plt.savefig(save_file_v6)
                    if self.close:
                            plt.show(block=False)  # Show without blocking execution
                            plt.pause(1)  # Keep open for 3 seconds
    ...
    def subtreshold_movement(self,  delta_n_stop = 2, delta_n_start = 3):
        
        df_sub_mov = pd.DataFrame(columns= ["Mice_id", "Trial", "Stop time [s]", "Event marker stop", "Move type", "Stop amplitude [mm]","Start time [s]", "Event marker start", "Average velocity [cm/s]", "Move duration [s]", "Start amplitude [mm]"]) 
        main = tk.Tk()
        msg = tk.messagebox.askquestion ('Save window','Do you want to save graphs and data?',icon = 'warning')
        if msg == "yes":
            main.destroy()
        ...
        for l,i in enumerate(self.list_of_df):
            ...
            if self.opto:
                ...
                start_index = [i.loc[i["TrialCt"] == hh].first_valid_index() for hh in trial_list if i.loc[(i["TrialCt"] == hh) & (i["Event_Marker"] == 1), "Amplitude_Pos"].tolist()]
                movment_end = [i.loc[(i["TrialCt"] == ii) & (i["Event_Marker"] == 1), "Amplitude_Pos"].first_valid_index() for ii in trial_list if i.loc[(i["TrialCt"] == ii) & (i["Event_Marker"] == 1), "Amplitude_Pos"].tolist()]
                trial_list_v2 = [trial for trial in trial_list if i.loc[(i["TrialCt"] == trial) & (i["Event_Marker"] == 1), "Amplitude_Pos"].tolist()]
                
                for trial_number, range_index in enumerate(zip(start_index, movment_end)):
                    #print(trial_number, range_index)
                    n_forward_start = 0
                    n_backward_start = 0
                    n_stop = 0
                    n_backward_stop = 0
                    ...
                    move_to_1_controller = True
                    move_4_controler_forward = False
                    move_4_controler_backward = False
                    ongoing_move_forward = False
                    ongoing_move_backward = False
                    for current_index in range(range_index[1], range_index[0] -1, -1):
                        current_delta = i.iloc[current_index, 18]
                        ...
                        if move_to_1_controller: #  Last move (to 1- above treshold) stop time
                            data_holder = self.data_stop(i, current_index, range_index, l, "To_treshold")
                            move_to_1_controller = False
                            ongoing_move = True 

                        
                        elif (i.iloc[current_index, 5] < 10 and i.iloc[current_index, 1] == 0) or  n_stop >= delta_n_stop : # move start -> Not to 1 
                                if n_stop >= delta_n_stop:
                                    current_index = current_index + delta_n_stop
                                if ongoing_move:
                                    #start_movement_time =  i.iloc[current_index +1, 0]
                                    ongoing_move = False
                                    move_4_controler_forward = True
                                    move_4_controler_backward = True
                                    #print(start_movement_time, stop_movement_time, "stop first from stoop move")
                                    start_movement_time =  i.iloc[current_index-1, 0]
                                    if data_holder[2] == start_movement_time:
                                        current_index -=1
                                    data_holder = self.data_start(i, current_index, data_holder)
                                    df_sub_mov.loc[len(df_sub_mov)] = data_holder
                                if ongoing_move_forward:
                                    ongoing_move_forward = False
                                    move_4_controler_forward = True
                                    #start_movement_time =  i.iloc[current_index, 0]
                                    #print(start_movement_time, stop_movement_time, "stop forward from stoop move")
                                    data_holder = self.data_start(i, current_index, data_holder)
                                    df_sub_mov.loc[len(df_sub_mov)] = data_holder
                                if ongoing_move_backward:
                                    ongoing_move_backward = False
                                    move_4_controler_backward = True
                                    #start_movement_time =  i.iloc[current_index, 0]
                                    #print(start_movement_time, stop_movement_time, "stop backward from stoop move")
                                    data_holder = self.data_start(i, current_index, data_holder)
                                    df_sub_mov.loc[len(df_sub_mov)] = data_holder
                                n_stop = 0
                               
                        elif n_forward_start >= delta_n_start and move_4_controler_forward: # forward move stop
                            if ongoing_move_backward:
                                #start_movement_time =  i.iloc[current_index + delta_n_stop, 0]
                                ongoing_move_backward = False
                                #print(start_movement_time, stop_movement_time, "stop backward from start forward move")
                                data_holder = self.data_start(i, current_index + delta_n_stop, data_holder)
                                df_sub_mov.loc[len(df_sub_mov)] = data_holder
                            move_4_controler_forward = False
                            #stop_movement_time = i.iloc[current_index + delta_n_start -1, 0]
                            ongoing_move_forward = True
                            data_holder = self.data_stop(i, current_index + delta_n_start -1, range_index, l, "Forward")
                            

                        
                        elif n_backward_start >= delta_n_start and move_4_controler_backward: # backward move stop
                            if ongoing_move_forward:
                                #start_movement_time =  i.iloc[current_index + delta_n_stop, 0]
                                ongoing_move_forward = False
                                move_4_controler_forward = True
                                #print(start_movement_time, stop_movement_time, "stop forward from start backward move")
                                data_holder = self.data_start(i, current_index + delta_n_stop, data_holder)
                                df_sub_mov.loc[len(df_sub_mov)] = data_holder
                            move_4_controler_backward = False
                            #stop_movement_time = i.iloc[current_index + delta_n_start-1, 0]
                            ongoing_move_backward = True
                            data_holder = self.data_stop(i, current_index + delta_n_start -1, range_index, l, "Backward")
                            
                        
                        elif n_backward_stop >= delta_n_stop and ongoing_move_backward:
                            #start_movement_time =  i.iloc[current_index + delta_n_stop, 0]
                            ongoing_move_backward = False
                            move_4_controler_backward = True
                            n_backward_stop = 0
                        ...
                        elif n_forward_stop >= delta_n_stop and ongoing_move_forward:
                            #start_movement_time =  i.iloc[current_index + delta_n_stop, 0]
                            ongoing_move_forward = False
                            move_4_controler_forward = True
                            n_forward_stop = 0
                        ...
                        elif n_forward_stop >= delta_n_stop and ongoing_move:
                            #start_movement_time =  i.iloc[current_index + delta_n_stop, 0]
                            ongoing_move = False
                            move_4_controler_forward = True
                            move_4_controler_backward = True
                            #print(start_movement_time, stop_movement_time, "stop first from backward or mix move")
                            
                            n_forward_stop = 0
                            data_holder = self.data_start(i, current_index + delta_n_stop, data_holder)
                            df_sub_mov.loc[len(df_sub_mov)] = data_holder
        df_sub_mov = df_sub_mov[["Mice_id", "Trial", "Start time [s]", "Stop time [s]", "Event marker start", "Event marker stop", "Move type",  "Average velocity [cm/s]", "Move duration [s]", "Start amplitude [mm]", "Stop amplitude [mm]"]]
        df_sub_mov_mean = self.sub_mov_process(df_sub_mov)
        if msg == "yes":
            df_sub_mov.to_excel( save_file_v1 + "\\" + self.list_of_files[0] + "_" + self.list_of_files[-1] + "_subtreshold_move_all" + ".xlsx")
            df_sub_mov_mean.to_excel(save_file_v1 + "\\" + self.list_of_files[0] + "_" + self.list_of_files[-1] + "_subtreshold_move_mean" + ".xlsx")

    def data_stop(self, df, c_index, range_index, l, move_type):
        data_holder = []
        animal = self.list_of_files[l]
        trial = df.iloc[range_index[0], 2]
        stop_movement_time = df.iloc[c_index, 0]
        stop_amplitude_pos = df.iloc[c_index,16]
        event_stop = df.iloc[c_index, 1]
        type_move = move_type
        [data_holder.append(item) for item in [animal, trial, stop_movement_time, event_stop, type_move, stop_amplitude_pos,c_index]]
        return data_holder

    def data_start(self, df, c_index,  data_holder):
        start_movement_time =  df.iloc[c_index, 0]
        event_start = df.iloc[c_index, 1]
        
        average_velo = df.iloc[c_index:data_holder[-1] +1, 19].mean()
        move_duration = data_holder[2] - start_movement_time
        start_amplitude_pos = df.iloc[c_index,16]
        data_holder.pop()
        [data_holder.append(item) for item in [start_movement_time, event_start, average_velo, move_duration, start_amplitude_pos]]
        return data_holder
    
    def sub_mov_process(self, data_holder):
        df_sub_mov_all = pd.DataFrame(columns= ["Mice_id", "Move type", "Average velocity [cm/s]", "Average move duration [s]", "Number of moves [n]", "Ratio move type / move all [%]",  "Average start amplitude [mm]", "Average stop amplitude [mm]"])
        for mice_id in data_holder["Mice_id"].unique():
            for move_type in data_holder["Move type"].unique():
                container = []
                avg_velo = data_holder.loc[(data_holder["Mice_id"] == mice_id) & (data_holder["Move type"] == move_type), "Average velocity [cm/s]"].mean()
                avg_duration = data_holder.loc[(data_holder["Mice_id"] == mice_id) & (data_holder["Move type"] == move_type), "Move duration [s]"].mean()
                n_move = len(data_holder.loc[(data_holder["Mice_id"] == mice_id) & (data_holder["Move type"] == move_type), :])
                n_move_ratio = round(len(data_holder.loc[(data_holder["Mice_id"] == mice_id) & (data_holder["Move type"] == move_type), :]) / len(data_holder.loc[data_holder["Mice_id"] == mice_id, :]) *100,2)
                avg_start_ampli = data_holder.loc[(data_holder["Mice_id"] == mice_id) & (data_holder["Move type"] == move_type), "Start amplitude [mm]"].mean()
                avg_stop_ampli = data_holder.loc[(data_holder["Mice_id"] == mice_id) & (data_holder["Move type"] == move_type), "Stop amplitude [mm]"].mean()
                [container.append(item) for item in [mice_id, move_type, avg_velo, avg_duration, n_move, n_move_ratio, avg_start_ampli, avg_stop_ampli]]
                df_sub_mov_all.loc[len(df_sub_mov_all)] = container
        for move_type in data_holder["Move type"].unique():
            container = []
            avg_velo = df_sub_mov_all.loc[df_sub_mov_all["Move type"] == move_type, "Average velocity [cm/s]"].mean()
            avg_duration = df_sub_mov_all.loc[df_sub_mov_all["Move type"] == move_type, "Average move duration [s]"].mean()
            avg_num = round(df_sub_mov_all.loc[df_sub_mov_all["Move type"] == move_type, "Number of moves [n]"].mean())
            avg_num_ratio = round(df_sub_mov_all.loc[df_sub_mov_all["Move type"] == move_type, "Ratio move type / move all [%]"].mean(),2)
            avg_start_ampli = df_sub_mov_all.loc[df_sub_mov_all["Move type"] == move_type, "Average start amplitude [mm]"].mean()
            avg_stop_ampli = df_sub_mov_all.loc[df_sub_mov_all["Move type"] == move_type, "Average stop amplitude [mm]"].mean()
            [container.append(item) for item in ["Mean", move_type, avg_velo, avg_duration, avg_num, avg_num_ratio ,avg_start_ampli, avg_stop_ampli]]
            df_sub_mov_all.loc[len(df_sub_mov_all)] = container
        return df_sub_mov_all
    
    def main_mov_process(self, data_holder):
        df_main_mov_all = pd.DataFrame(columns= ["Mice_id", "Move type", "Average move duration [s]", "Number of moves [n]", "Ratio move_type / move_all [%]", "Average move latency [s]"])
        for mice_id in data_holder["Mouse_ID"].unique():
            for move_type in data_holder["move type"].unique():
                container = []
                avg_duration = data_holder.loc[(data_holder["Mouse_ID"] == mice_id) & (data_holder["move type"] == move_type), "move_duration [sec]"].mean()
                n_num = len(data_holder.loc[(data_holder["Mouse_ID"] == mice_id) & (data_holder["move type"] == move_type), "move_duration [sec]"])
                n_ratio = round(len(data_holder.loc[(data_holder["Mouse_ID"] == mice_id) & (data_holder["move type"] == move_type), "move_duration [sec]"]) / len(data_holder.loc[data_holder["Mouse_ID"] == mice_id, "move_duration [sec]"])*100,2)
                avg_latnecy = data_holder.loc[(data_holder["Mouse_ID"] == mice_id) & (data_holder["move type"] == move_type), "move latency [sec]"].mean()
                [container.append(item) for item in [mice_id, move_type,  avg_duration, n_num, n_ratio, avg_latnecy]]
                df_main_mov_all.loc[len(df_main_mov_all)] = container
        for move_type in data_holder["move type"].unique():
            container = []
            if self.opto:
                stim_phase = ["_prestim", "_stim", "_poststim"]
                
                for phase in stim_phase:
                    container = []
                    avg_duration = df_main_mov_all.loc[(df_main_mov_all["Move type"] == move_type) & (df_main_mov_all["Mice_id"].str.contains(phase, case=False, na=False)),"Average move duration [s]"].mean()
                    n_num = round(df_main_mov_all.loc[(df_main_mov_all["Move type"] == move_type) & (df_main_mov_all["Mice_id"].str.contains(phase, case=False, na=False)), "Number of moves [n]"].mean(),2)
                    n_ratio =round(df_main_mov_all.loc[(df_main_mov_all["Move type"] == move_type) & (df_main_mov_all["Mice_id"].str.contains(phase, case=False, na=False)), "Ratio move_type / move_all [%]"].mean(),2)
                    avg_latency = df_main_mov_all.loc[(df_main_mov_all["Move type"] == move_type) & (df_main_mov_all["Mice_id"].str.contains(phase, case=False, na=False)),"Average move latency [s]"].mean()
                    [container.append(item) for item in ["Mean" + phase, move_type,  avg_duration, n_num, n_ratio, avg_latency]]
                    df_main_mov_all.loc[len(df_main_mov_all)] = container
            else:
                avg_duration = df_main_mov_all.loc[df_main_mov_all["Move type"] == move_type,"Average move duration [s]"].mean()
                n_num = round(df_main_mov_all.loc[df_main_mov_all["Move type"] == move_type, "Number of moves [n]"].mean(),2)
                n_ratio =round(df_main_mov_all.loc[df_main_mov_all["Move type"] == move_type, "Ratio move_type / move_all [%]"].mean(),2)
                avg_latency = df_main_mov_all.loc[df_main_mov_all["Move type"] == move_type,"Average move latency [s]"].mean()
                [container.append(item) for item in ["Mean", move_type,  avg_duration, n_num, n_ratio, avg_latency]]
                df_main_mov_all.loc[len(df_main_mov_all)] = container
        return df_main_mov_all
...
object_joy = Joystick_analyzer(opto=True, close= False)
object_joy.pre_proccesing( cut_first=True)
object_joy.find_bugs(alfa = 0.1, automatic = False)
#object_joy.veloctiy(group = "all", y_lim = [0, 20])
object_joy.amplitude(hue = "Event_Marker",group = None, event_markers = [1,2,4], x_lim= [4.5,12.5], y_lim= [], x_axis = "mm", fill_nan= False) 
#object_joy.move_type(event_markers = [0,1,3,4], hue = "Event_Marker", group = "all")
#object_joy.help_me()
#object_joy.prob_reward()
#object_joy.amplitude_time(group = "all")
#object_joy.lick_histogram(stat = "rate", group = "all", y_lim = [0 , 17], sem = True)
#object_joy.trajectory(move_range = "start_move_to_max", calibrate= False, polar = False, delta= True, delta_n= 5)
#object_joy.subtreshold_movement(delta_n_start= 3, delta_n_stop= 2)
